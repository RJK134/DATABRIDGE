...
export const H05_RULES: RuleDefinition[] = [
  ...
  },
  {
    ...
    evaluate: (record) => {
      if (!record['engagementId'] && !record['studentId']) {
        return {
          pass: false,
          message: 'StudentCourseSession has neither engagementId nor studentId — cannot link to an Engagement.',
        };
      }
      return { pass: true };
    },
  ...
  },
  {
    ...
    description: 'If engagementEndDate is populated, reasonForEnding (RSNEND) must also be populated',
    evaluate: (record) => {
      ...
      if (endDate && !rsnend) {
        return {
          pass: false,
          message: 'RSNEND (reasonForEnding) must be provided when an engagement end date is recorded.',
        };
      }
      return { pass: true };
    },
