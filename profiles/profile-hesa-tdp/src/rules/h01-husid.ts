...
export const H01_RULES: RuleDefinition[] = [
  {
    ...
    evaluate: (record: Record<string, unknown>) => {
      if (!record["husid"]) {
        return {
          pass: false,
          message: "HUSID is missing. Every student must have a HESA Unique Student Identifier.",
        };
      }
      return { pass: true };
    },
  ...
  },
  {
    ...
    evaluate: (
      ...
      context?: { allRecords?: Record<string, unknown>[] }
    ) => {
      if (!context?.allRecords) return { pass: true };
      const husid = record["husid"];
      if (!husid) return { pass: true };
      const duplicates = context.allRecords.filter((r) => r !== record && r["husid"] === husid);
