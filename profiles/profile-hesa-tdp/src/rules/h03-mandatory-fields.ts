  ...
  },
  {
    ...
    evaluate: (record) => {
      if (!record['hecosSubject1'] || String(record['hecosSubject1']).trim() === '') {
        return { pass: false, message: 'HECOS1 (hecosSubject1) is missing. Every course session must have a primary HECoS subject code.' };
      }
      return { pass: true };
    },
  },
  ...
  },
  {
    ...
    evaluate: (record) => {
      if (!record['hecosSubject'] || String(record['hecosSubject']).trim() === '') {
        return { pass: false, message: 'MODHECOS (hecosSubject) is missing. Every module must have a HECoS subject code.' };
      }
      return { pass: true };
    },
  },
