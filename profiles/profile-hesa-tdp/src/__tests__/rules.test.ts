...
describe('coding frames', () => {
  ...
  it('all coding frames are exported in CODING_FRAMES registry', () => {
    const expected = ['ETHNIC', 'DISABLE', 'DOMICILE', 'MODE', 'RSNEND', 'QUALENT3', 'FUNDCOMP', 'SEXID'];
    for (const name of expected) {
      expect(CODING_FRAMES[name as keyof typeof CODING_FRAMES], `${name} missing from CODING_FRAMES`).toBeTruthy();
    }
  });
});
