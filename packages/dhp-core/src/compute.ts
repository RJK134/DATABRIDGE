...
import type { DhpComputeInput, DhpDimension, DhpEntityMetrics, DhpMetrics } from './types';
...
export function computeDhp(input: DhpComputeInput): DhpMetrics {
  ...
  const overallScore = parseFloat(
    (entities.reduce((sum, e) => sum + e.overallScore, 0) / Math.max(1, entities.length)).toFixed(
      4,
    ),
  );
